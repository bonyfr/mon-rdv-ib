...
public class LieuDaoTest {
	...
	public void Lieu() throws ParseException {
		...
		Adresse adresseFind = adresseDao.findById(lieuFind.getAdr().getId()).get();
		...
		Utilisateur utilisateurFind = utilisateurDao.findById(lieuFind.getPraticien().getId()).get();
		...
		if (optLieuFind.isPresent()) {
			Assert.fail("Erreur Delete Lieu");
		}
