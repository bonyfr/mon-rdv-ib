...
public class AdresseDaoTest {
	...
	public void adresse() {
		...
		Optional<Adresse> optAdresseFind = adresseDao.findById(idAdresse);
		...
		optAdresseFind = adresseDao.findById(idAdresse);
		
		if(!optAdresseFind.isPresent()) {
			Assert.fail("Erreur find adresse");
		}
		
		adresseFind = optAdresseFind.get();
		...
		if(optAdresseFind.isPresent()) {
			Assert.fail("Erreur à la suppression");
		}
